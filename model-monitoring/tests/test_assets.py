...
@pytest.fixture
def fake_recent_interval(request) -> pandas:  # pylint: disable=missing-function-docstring
...
def test_reference_accidents_dataset(monkeypatch, fake_dataset):  # pylint: disable=missing-function-docstring
...
def test_recent_interval_processor(
    ...
    mocker.patch(
        "workflows.assets.read_pandas_asset", 
        return_value=fake_recent_interval
    )
...
@pytest.mark.parametrize(
        ...
        [
            ("1", 2, "2000-01-03", "2000-01-04"),
            ("2", 3, "2000-01-09", "2000-01-11")
        ],
        indirect=["fake_recent_interval"]
)
def test_recent_accidents_dataset(
        mocker,
        fake_recent_interval,
        output_len,
        output_min_date, 
