...
def reference_accidents_predictions(
    ...
    logger = get_dagster_logger()
    logger.info(
        f"Requestion predictions from {os.environ['PREDICT_URL']}"
    )
...
def recent_accidents_predictions(
    context,
...
def recent_evidently_report(
    ...
    dtypes = infer_feature_types(
        reference_accidents_dataset,
        skip=["date", "accident.accident_index"]
    )
    ...
    logger.info(
        f"Creating reports for date interval: [{start_date, end_date}]"
    )
    ...
    for date in pandas.date_range(start=start_date, end=end_date):
        ...
        logger.info(
            f"Creating report for {date} with {len(eval_single)} events."
        )
